...
public class AIVisionComponent : MonoBehaviour
{
    ...
    public EdgeInfo GetEdges()
    {
        ...
        Debug.Log(edge.pointA);
        Debug.Log(edge.pointB);
        return edge;
    }
    ...
    private void FindVisibleTargets()
    {
        ...
        Collider[] targetsInViewRadius = Physics.OverlapSphere(Eyes.position, viewRadius, targetMask);
        ...
        for (int i = 0; i < targetsInViewRadius.Length; i++)
        {
            ...
            {
                float dstToTarget = Vector3.Distance(Eyes.position, target.transform.position);
                if (!Physics.Raycast(Eyes.position, dirToTarget, dstToTarget, obstacleMask))
                {
                    visibleTargets.Add(target.attachedRigidbody.transform);
                }
            }
        }
    }
    ...
    private void DrawFieldOfView()
    {
        ...
        {
            vertices[i + 1] = Eyes.InverseTransformPoint(viewPoints[i]) + Vector3.forward * maskCutawayDst;
    ...
    private ViewCastInfo ViewCast(float globalAngle)
    {
        ...
        if (Physics.Raycast(Eyes.position, dir, out hit, viewRadius, obstacleMask))
        {
